import { useState, ReactElement, useEffect } from 'react';
  ...
  const cryptos = [
    ...
    },
    {
      ...
      icon: (
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M6.5 7.5L12 13L17.5 7.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M6.5 16.5L12 11L17.5 16.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      ),
    },
  ];
    ...
    if (selectedCrypto) {
      const fetchData = async () => {
        ...
        try {
          ...
          const response = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${selectedCryptoData.id}&vs_currencies=usd&include_24hr_change=true`);
        ...
        <div className="px-4 py-2 rounded-lg min-w-[200px] mb-2">
          <div className="text-white text-center">
            ...
            {isLoading ? (
              ...
              <>
                <div className="text-lg font-bold">${cryptoData.price?.toLocaleString() ?? 'N/A'}</div>
                <div className={`text-sm ${(cryptoData.change24h ?? 0) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                  {(cryptoData.change24h ?? 0) >= 0 ? '+' : ''}{cryptoData.change24h?.toFixed(2) ?? '0.00'}%
                </div>
              </>
            ) : (
